	...
	getHotNews : function(categoryId) {
		...
		$.getJSON(GLOBAL_APP_CONTEXT + "/synd/hotNews/" + categoryId, function(
			...
			$.each(data, function(cnt, item) {
				...
				newsContainer.append("<div class='newsTitle'>" + item.title
						+ "</div>");
				newsContainer.append("<div class='newsDate'>" + item.updateDate
						+ "</div>");
				newsContainer.append("<div class='newsDesc'>"
						+ item.description + "</div>");
				newsContainer.append("<div class><a href='" + item.uri
